...
# ⚠ 컨테이너에서 이 파일이 정본인 것은 **`docker run` 경로뿐이다.**
#   `docker-entrypoint.sh` 의 가드가 `[ -n "$FASTDDS_BUILTIN_TRANSPORTS" ] || source`
#   라서, `docker-compose.yml` 이 `environment:` 로 먼저 채우는 compose 경로에서는
#   **이 파일이 한 번도 source 되지 않는다** — 적용되는 값은 compose 쪽 사본이다.
#   그래서 위 사본 목록에서 `docker-compose.yml` 이 안 빠진다. 두 값이 갈리면
#   `camera_info` 만 오고 이미지가 조용히 안 온다 — 라이브 재생에서 실제로 걸렸다
#   (docs/shared_server_2026-08-31.md §9-3).
