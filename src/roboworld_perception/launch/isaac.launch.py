...
_PRESET = [
    ...
    #
    # 지금 672 를 쓰는 실제 이유는 **속도**다. 현재 HEAD 라이브 실측
    # (2026-08-24 저녁, 전제 검사 통과): 672 에서 **4.72 Hz**, 1008 에서
    # **2.72 Hz (-42.4%)**. Isaac 실시간은 이미 느려서(detect_interval=1 인
    # 이유가 그것) 2.7 Hz 면 추적기가 설 자리를 더 잃는다.
    # VRAM 은 판단 근거가 못 된다 — 1008 에서도 2.7GB 가 남는다.
    # (17:04 의 4.98 / 2.68 은 이후 폐기된 빌드 값이다 — 구독 QoS 가 빠지면서
    #  672 만 5% 손해를 봤다. 상세는 docs/image_size_2026-08-21.md ③)
    #
