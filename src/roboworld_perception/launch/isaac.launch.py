...
_PRESET = [
    ...
    # rviz=false 다. 1008 로 올려도 VRAM 피크 9,797 MiB 로 12GB 중 2,485 MiB
    # 가 남는다. **VRAM 은 병목이 아니다.**
    #
    # 지금 672 를 쓰는 실제 이유는 **속도**다. 라이브 재측정(2026-08-24,
    # 브리지 복구 후, 전제 검사 통과·sync_queue_size=1 이후): 672 에서
    # **4.98 Hz**, 1008 에서 **2.68 Hz (-46.1%)**. 큐를 1 로 줄이면 느린 쪽이
    # 프레임을 버려 격차가 좁아질 줄 알았는데 반대였다 — 672 만 개선됐다.
    # Isaac 실시간은 이미 느려서(detect_interval=1 인 이유가 그것) 2.7 Hz 면
    # 추적기가 설 자리를 더 잃는다.
    # VRAM 은 이제 판단 근거가 못 된다 — 1008 에서도 3,413 MiB 가 남고
    # 올리는 비용 자체가 +178 MiB 뿐이다.
    #
    ...
    # 어휘는 속도 비용이 0 이고, 896 으로 올리는 것은 위 41% 를 내는 길이다.
    #
    ...
    # 길이는 194.9 / 195.7 로 0.8mm 차이인데 이는 실행 간 평면 변동(±0.40mm)과
    # 같은 자리수라 해상도 효과로 분리되지 않는다. 두께는 57.8 / 57.7 로
    ...
    ("max_per_prompt", "10"),
    # **껐다. 이 게이트는 조각을 0 프레임 막으면서 정상을 90 프레임 잘랐다.**
    #
    # 원래 근거는 유효했다 - 2026-08-19, 43 사이클에서 두 무리가 갈렸다:
    #     정상 블록 7 개  score 0.840~0.945   최대 extent 0.152~0.198
    #     허수 조각 2 개  score 0.416~0.432   최대 extent 0.0447~0.0498
    # score_threshold(0.4)는 신규 트랙 생성 문턱일 뿐이라 조각이 0.416/0.432 로
    # 그대로 통과했고, 두 무리 사이(0.43~0.84)에서 0.6 을 골랐다.
    #
    # **그 뒤 조각을 실제로 없앤 것은 이 게이트가 아니라 enable_merge 와 조각
    # 수정이었다.** 그 결과 남은 소크기 트랙이 고득점 하나만 남아 분리 변수가
    # score 에서 크기로 옮겨갔다. 게이트 off CSV 재검증(2026-08-20 output/belt_*,
    # 채택 어휘):
    #     T6  ext 0.203  정상 블록          최저 score 0.135
    #     T8  ext 0.168  실블록, 경계 절단   최저 score 0.102
    #     T11 ext 0.073  **진짜 조각**       최저 score **0.645**
    # 정상이 0.135 까지 내려가는데 조각은 0.645 아래로 안 내려간다 - **정상을
    # 살리면서 조각을 막는 문턱이 존재하지 않는다.** 현행 프리셋(merge on)에서
    # 0.6 의 실제 손익은 정상/실블록 90 프레임 손실 대 조각 0 프레임 차단이다.
    #
    # 0.0 으로 내려도 조각이 돌아오지 않는다 - 조각을 막고 있는 것은 위의
    # enable_merge=true 다. 상세는 docs/datasets.md 의 "어휘를 바꾸기 전에" 절.
    #
    # **어휘를 바꾸는 사람에게**: 이 게이트를 다시 켜려면 게이트 off 로 뽑은
    # CSV 로 실블록(ext>=0.15) per-track 최저 score 를 재측정하고 그보다 낮은
    # 값을 골라야 한다. 게이트를 켜고 뽑은 CSV 로는 정할 수 없다 -
    # run_offline.py 가 publishable 로 걸러 질문이 사는 자리를 잘라낸다.
    ("publish_score_min", "0.0"),
