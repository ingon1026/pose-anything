...
_PRESET = [
    ...
    # 실측 23,410 -> 0 회. VRAM 근거는 확실하다.
    #
    # **단, "카메라가 640x360 이라 1008 은 어차피 업스케일이니 무해하다" 는
    # 직관은 틀렸다** (2026-08-21 실측). test2 에서 672 로 낮추면 스마트폰
    # (겉보기 최단축 39px)이 **통째로 사라진다** — 소실 경계는 800~896 이고,
    # 큰 물체(109~117px)는 672 에서도 멀쩡하다. 소물체만 선택적으로 죽고
    # 에러가 없어서 알아채기 어렵다. docs/image_size_2026-08-21.md
    #
    # 지금 이 씬은 블록만 있어서(겉보기 33px 인데 살아남는다 — 이유 미확인)
    # 성립하지만, **작은 물체를 추가하면 조용히 사라진다.** 그때는 896 이
    # 확인된 안전 하한이다(VRAM 은 (896/672)^2 = 1.78 배 늘어난다).
    ("image_size", "672"),
