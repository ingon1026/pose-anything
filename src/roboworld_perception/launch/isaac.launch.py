...
_PRESET = [
    ...
    #
    # 지금 672 를 쓰는 실제 이유는 **속도**다. 검출 발행률이 672 에서 4.60 Hz,
    # 1008 에서 2.70 Hz (-41%). Isaac 실시간은 이미 느려서(detect_interval=1
    # 인 이유가 그것) 2.7 Hz 면 추적기가 설 자리를 더 잃는다.
    #
    ...
    #
    # **치수는 폭만 1008 보다 3.0mm 작게 잡힌다** (isaac_belt_moving
    # 500프레임: 672 -> 46.8mm, 1008 -> 49.8mm). 저해상도에서 마스크가
    # 줄어서이고, 마스크 점 개수도 6% 적다(515 vs 550). 단 **블록 폭의 정답
    # 치수는 없어서 어느 쪽이 맞는지는 모른다** — 아는 것은 3.0mm 차이뿐이다.
    # 두께는 672/1008 이 57.8/57.7mm 로 같고 둘 다 정답(57.3) +0.5mm —
    # **두께는 해상도와 무관하다.** 그리퍼 허용 오차가 3mm 보다 타이트하면
    # 1008 을 검토할 것. 상세는 docs/image_size_2026-08-21.md
    #
