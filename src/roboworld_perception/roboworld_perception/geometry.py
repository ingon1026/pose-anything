...
def obb_on_plane(points, plane, min_thickness=MIN_THICKNESS, top_q=0.98,
    ...
    **풋프린트는 축마다 1픽셀 짧다 (2026-08-25, 열린 항목).** _backproject 가
    ...
    isaac_belt_moving + image_size 1008(운영 기본)에서 이것이 풋프린트 부족의
    **지배 항**이다 (fx 322.86, z 0.945m -> 2.93mm). 정답 200x55mm 에 대한
    분해(166관측):
    ...
    **두 항의 처방이 같다 — 부분화소 경계다.** 규약 1px 과 래스터 0.79px 둘 다
    ...
    마스크 몫은 1008 에서 폭 1mm 이하다. **단 이 상한은 image_size 에 딸린다** —
    규약 몫 2.93mm 는 원본 래스터에서 나와 두 arm 에 공통이므로, 672 의 관측
    부족 8.2mm 에서 그것과 depth 항을 빼면 마스크 몫이 약 4mm 로 추정된다
    (1008 의 4배). **단 그 4mm 는 depth 항을 -1.27 로 놓은 값이고 그 항은
    [-1.9, +0.3] 로 미확정이다** — 구간 끝에서 두 arm 비가 1.44~6.07x 로
    갈리므로 척도 의존의 크기는 아직 못 정한다. 그래서 image_size 672 vs
    1008 결정은 이 절로 닫히지 않는다 — docs/README.md §4.
    ...
    **그래도 +1px 를 더하면 안 된다 — 오차가 각도 의존이다.** 합성 래스터
    ...
    이 bag 의 블록이 축 정렬이라(minAreaRect 각 중앙값 0.000deg, 95백분위
    0.165deg) -2.93mm 가 온전히 걸린 것이다. 일률 보정은 12~31deg 물체를
    최대 +3mm 과보정한다. 필요한 것은 부분화소 경계이거나 각도 인지 보정이고,
    출하 크기 전부가 바뀌는 변경이라 아직 안 넣었다 — docs/README.md §4.
