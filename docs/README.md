...
| 항목 | 상태 | 출처 |
|---|---|---|
| **`[belt_plane]` 로그에 인라이어 비율 + 잔차 RMS 추가** | `pipeline.py:384-385` 가 지금은 평면 튜플/실패 문자열만 찍는다. **줄은 이미 있고 내용만 바뀌므로 비용 0** 이고, 이걸로 13mm 씬이 눈에 보인다. **게이트를 넣기 전에 로그부터** — 실 bag 분포를 모은 뒤 문턱을 정하는 것이 순서다. `src/` 라 사용자 몫 | [belt_plane](belt_plane_2026-08-21.md) "권고 순서" |
