...
| `image_size` | 기본 1008, **Isaac 672** | [image_size](image_size_2026-08-21.md) ③ — 근거는 VRAM 이 아니라 **속도**(4.60 vs 2.70 Hz). **단 `sync_queue_size` 변경 전 측정** → 조건이 바뀌면 이 결정도 다시 본다 |
...
| `image_size` 672 vs 1008 | 속도(4.60 vs 2.70 Hz — **큐 변경 전 측정**) ↔ **폭** 3.0mm(정답 55.0 대비 −8.2 vs −5.2, 오프라인 재생·`geometry.py` 무변경이라 유효). 두께는 무관 | [image_size](image_size_2026-08-21.md) ③④ |

### 측정하면 답이 나오는 것
| 항목 | 출처 |
|---|---|
| **`sync_queue_size=1`(08-24 오후) 이후의 검출 발행률** — 늦은 프레임을 버리게 돼 1008 쪽 실효 발행률이 달라질 수 있다. −41% 는 `image_size` 672 의 유일한 근거다 | [image_size](image_size_2026-08-21.md) ③ |
...
- **수치를 인용할 때 조건을 같이 적을 것.** 같은 양이 조건에 따라 크게 다르다:
  ...
  - **속도·발행률 — 동기화 큐 설정과 torch/torchao 상태를 같이 적을 것.**
    `image_size` 672 의 **유일한 근거**인 4.60 vs 2.70 Hz(−41%)는
    ([image_size](image_size_2026-08-21.md) ③) **`sync_queue_size=1`·늦은 프레임
    폐기 변경(08-24 16:06~16:09) 이전** 라이브 측정이다 — 큐가 1 이면 느린
    쪽(1008)이 밀린 프레임을 버리므로 비율이 달라질 수 있다. **재측정 전까지
    이 Hz 를 조건 없이 인용하지 말 것.** 같은 실행 로그 첫 줄에
    ...
    **이건 `torchao 0.17.0` 이 내는 경고이고 빠지는 것은 그 양자화 커널이지
    ...
    업그레이드가 급한 일로 오해된다** — 급하지 않다. 다만 근거가 속도 하나뿐이라
    어느 조건이 바뀌든 한 번은 다시 재야 한다
