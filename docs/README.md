...
| 문서 | 요지 |
|---|---|
| [belt_plane_2026-08-21.md](belt_plane_2026-08-21.md) | 벨트 평면 구속 OBB. 두께 붕괴·광축 편향 해소. **거기 적힌 "정답 +0.7mm" 는 파생 측정 기준** — 모델(55.0mm) 대비로는 +2.7mm |
...
>
> 그리고 **"정답" 이 모델인지 파생 측정인지 구분할 것.** belt_plane 이 쓰는
> "정답 57.3mm" 는 depth 판독 두 개(벨트 1002.3 − 상면 945.0)에서 나온
> **측정값**이고, 씬 USD 의 모델 치수는 **200 × 55 × 55mm** 다. 파생 정답이
> 우리 측정과 같은 평면 편향(약 2.4mm)을 공유해서 "정답 +0.7mm" 로 맞아떨어졌다
> — **모델 대비로는 +2.7mm 다.** 정답을 측정에서 만들면 검증이 순환한다.
...
| `image_size` 672 vs 1008 | 속도(4.60 vs 2.70 Hz) ↔ **폭** 3.0mm(정답 55.0 대비 −8.2 vs −5.2). 두께는 무관 | [image_size](image_size_2026-08-21.md) ③④ |
...
- **수치를 인용할 때 조건을 같이 적을 것.** 같은 양이 조건에 따라 크게 다르다:
  ...
  - 치수 정확도 — **"정답" 이 모델인지 파생 측정인지 밝힐 것.** Isaac 블록의
    모델 치수는 **200 × 55 × 55mm**(씬 USD 직독)이고, belt_plane 의 "정답
    57.3mm" 는 depth 판독 두 개에서 파생된 측정값이다. 모델 대비 실측은
    풋프린트 −4~8mm(과소), 두께 +2.7mm(과대)다. **어느 축인지도 같이 적을 것**
