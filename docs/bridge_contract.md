...
⚠ **대역폭은 물체 크기에 비례한다 — 단일 수치로 인용하지 말 것.**
불변식은 **마스크 화소당 약 8 바이트**다(화소 2개당 점 1개 × 16 B — `stride=2`
는 1차원으로 솎으므로 감축이 1/4 이 아니라 1/2 다). 실측 두 점:

| 장면 | 마스크 | 객체당 |
|---|---|---|
| Isaac 블록 (2026-08-28 라이브) | ≈592 화소 | **4.6 KB** |
| 640×480 책·키보드급 (합성) | 7,310 화소 | **55.8 KB** |

**12배 폭이다.** 물체 3개면 대략 **15 KB ~ 170 KB/프레임**. 자기 장면에서
직접 재고 쓸 것.

⚠ **속도 비용.** publishable 트랙마다 점을 팩하고 직렬화한다. **2026-08-28
(`a631ec0`) 이전에는 `mask_depth_to_points` 를 프레임당 한 번 더 불렀는데
그 중복은 없앴다** — 이제 이번 프레임에 `_update_geometry` 를 탄 트랙은
파이프라인이 이미 만든 배열을 그대로 쓴다. 안 탄 트랙(동결·침입 보류)만
직접 계산한다. **재측정은 안 했다** — 예전에 적혀 있던 "≈10ms/트랙,
3개면 +15~30ms" 는 그 커밋 이전 값이라 지금은 과대평가다.
**관측 도구지 상시 기능이 아니다.**
...
가 필요하다.** 그래서 **기본값이 `rviz` 인자를 따라간다**(`0630520`) — RViz 를
여는 실행은 켜지고, 안 여는 실행(통합·headless)은 꺼진 채로 남는다. 공칭·비캘리브
값이라 필요 없는 곳에 내보내지 않으려는 것이고, 노드가 발행할 때 경고를 낸다.

**`isaac.launch.py` 는 프리셋이 `rviz=false` 라 기본으로는 꺼진다** —
`ros2 launch roboworld_perception isaac.launch.py rviz:=true` 로 켜야 TF 가 생긴다
(실측 2026-08-28: `world -> camera_link` z=1.0 발행 확인). 예전에 여기 적혀
있던 *"Isaac 프리셋은 켜 둔다"* 는 그 프리셋에 못박아 뒀던 때의 이야기이고,
`0630520` 이 중복이라 뺐다. **Isaac 경로에서 RViz 가 비어 보이면 이 줄부터 볼 것.**

`run.sh` 는 headless 가 아니면 켠다(`run.sh:116-126`).
...
구멍 바닥(z=0.983)이 위로 간다. **2026-08-28 에 "파인 부분이 본체보다 높아
보인다" 는 보고가 정확히 이것이었다** — 점군 값 자체는 옳았다(25프레임에서 전
블록 최고 높이 **55.0mm 동일**, 씬 정답 가시높이 54.5mm).

⚠ 이 절 앞부분과 어긋나 보이는 문장이 한동안 여기 있었다 — *"설정 파일은
`camera_link` 로 고쳤다"*. 그건 **중간 상태**다: `1007af4` 가 광학 프레임에서
`camera_link` 로 옮겼고, `71437fa` 가 다시 `world` 로 돌렸다(노드가
`perception_node.py:195-199` 에서 `world` 를 계약으로 못박고 있기 때문).
**현재 파일은 `perception.rviz:89` 가 `Fixed Frame: world` 다.**

**높이를 눈으로 판정하기 전에 Fixed Frame 부터 확인할 것.**
