...
| | 현행 이미지 경로 | 비디오 세션 |
|---|---|---|
| 키프레임 추론 | **217 ms** (1.2.0 A/B 중앙값) | **556~623 ms** |
| 검출 발행률 | **4.587 Hz** (2026-08-28 실측) | **약 1.7 Hz** |
...
```
**검출 NMS·구멍 메우기·잡티 제거가 통째로 스킵되고 있다.** frame 70 부터
두 번째 객체가 **56,876 → 70,586 화소**(프레임의 23%)로 자라는 현상의 유력한
원인이다. `pip install kernels` 로 닫히는지 **미확인.**
...
비디오 세션은 **1.7 Hz** 다 — 이미 기각된 2.68 Hz 보다 **37% 더 느리다.**
