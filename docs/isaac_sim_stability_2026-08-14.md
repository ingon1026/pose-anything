...
> ⚠ **위 표의 "남는 여유 2,265 MiB / 18%" 와 이 문단의 "81%" 는 총량 12,282 MiB 가
> 분모로 들어간 값이다** — 제목의 라벨(RTX 4070 Ti)이 그 조건이다. **통합 메모리
> 기계에서는 이 양들이 값만 달라지는 게 아니라 계산 자체가 안 된다**: DGX
> Spark(GB10)은 `nvidia-smi --query-gpu=memory.total` 이 `[N/A]` 다.
> **즉 이 문단이 세운 위험 기준(여유가 없는 것이 위험하다)을 거기서는 그대로 쓸 수
> 없다 — 무엇으로 여유를 볼지부터 다시 정해야 한다. 미확인.**
> → [shared_server](shared_server_2026-08-31.md) §1. 항목별 절대 사용량(5,055 ·
> 2,641 · 2,321 MiB)은 양은 정의되고 값만 바뀐다.
